...
export default function AdminCommunityImagesPage() {
  ...
  async function load() {
    const res = await apiFetch<{ images: CommunityImage[] }>('/api/community-images/admin/all', { withCredentials: true });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await apiFetch('/api/community-images/admin', { method: 'POST', body, withCredentials: true });
      setFile(null);
      setPreview(null);
      setCaption('');
  ...
  async function handleOrderChange(id: number, sortOrder: number) {
    await apiFetch(`/api/community-images/admin/${id}/order`, {
      method: 'PATCH',
  ...
  async function handleDelete(id: number) {
    ...
    await apiFetch(`/api/community-images/admin/${id}`, { method: 'DELETE', withCredentials: true });
    load();
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">Community Images</h1>
      ...
      <div className="mt-8">
        <h2 className="mb-4 text-base font-semibold text-neutral-800">
          All Photos <span className="ml-1 text-sm font-normal text-neutral-400">({images.length})</span>
        </h2>
            ...
            {images.map((img) => (
              <div key={img.id}>
                <div className="relative aspect-square overflow-hidden rounded-xl border border-neutral-200">
                  <Image src={img.image_url} alt={img.caption ?? ''} fill sizes="160px" className="object-cover" />
                </div>
                {img.caption && (
                  <p className="mt-1 truncate text-xs text-neutral-500">{img.caption}</p>
                )}
