      ...
      <section className="bg-white px-4 py-4 sm:px-6 sm:py-0">
        <div className="mx-auto grid max-w-2xl grid-cols-2 divide-x divide-y divide-neutral-200 overflow-hidden rounded-2xl border border-neutral-200">
          {[
            ...
            <div key={title} className="flex flex-col items-center px-3 py-3 text-center sm:px-6 sm:py-10">
              <span className="text-[#3B2A1C] [&>svg]:h-6 [&>svg]:w-6 sm:[&>svg]:h-8 sm:[&>svg]:w-8">{icon}</span>
              <h3 className="mt-2 text-[10px] font-bold uppercase tracking-[0.12em] text-[#2A1F14] sm:mt-4 sm:text-xs sm:tracking-[0.15em]">{title}</h3>
              <p className="mt-1 text-[10px] leading-relaxed text-neutral-500 sm:mt-2 sm:text-sm">{desc}</p>
            </div>
          ))}
        </div>
      </section>
