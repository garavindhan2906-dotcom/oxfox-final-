      ...
      <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${bgClass}`}>
        ...
        <div className="hidden items-center px-6 py-3.5 md:flex lg:px-8">
          ...
          <div className="flex flex-shrink-0 items-center gap-5">
            ...
            <UserProfileDropdown />
            <Link href="/faq" className="text-xs font-semibold uppercase tracking-wide text-neutral-600 hover:text-neutral-900">FAQs</Link>
      ...
      {mobileOpen && (
        <div className="fixed inset-x-0 top-[76px] bottom-0 z-[49] flex flex-col bg-[#F5EDE4] md:hidden overflow-y-auto pb-32">
            ...
            <nav className="flex flex-col gap-1.5 px-3">
              ...
              {[
                { href: '/', label: 'Home', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 0 0 1 1h3m10-11 2 2m-2-2v10a1 1 0 0 1-1 1h-3m-6 0a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1m-6 0h6" /> },
                { href: '/new', label: 'New Arrivals', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 0 0 .95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 0 0-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 0 0-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 0 0-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 0 0 .951-.69l1.519-4.674z" /> },
                { href: '/custom-order', label: 'Custom Order', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 1 1 3.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /> },
                { href: '/community', label: 'Community', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 0 0-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 0 1 5.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 0 1 9.288 0M15 7a3 3 0 1 1-6 0 3 3 0 0 1 6 0z" /> },
                { href: '/shipping', label: 'Shipping', icon: <><rect x="1" y="3" width="15" height="13" strokeLinecap="round" strokeLinejoin="round" /><path strokeLinecap="round" strokeLinejoin="round" d="M16 8h4l3 3v5h-7V8z" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></> },
                { href: '/about-us', label: 'About Us', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 0 0 0 6.364L12 20.364l7.682-7.682a4.5 4.5 0 0 0-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 0 0-6.364 0z" /> },
                { href: '/faq', label: 'FAQs', icon: <><circle cx="12" cy="12" r="10" strokeLinecap="round" strokeLinejoin="round" /><path strokeLinecap="round" strokeLinejoin="round" d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3M12 17h.01" /></> },
                { href: '/bulk-orders', label: 'Bulk Orders', icon: <><path strokeLinecap="round" strokeLinejoin="round" d="M20 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V8a1 1 0 0 0-1-1Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2" /></> },
              ].map(({ href, label, icon }) => (
                <Link key={href} href={href} onClick={close} className="flex items-center justify-between rounded-xl border border-[#EAE3DA] bg-white px-3 py-2.5 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[#3B2A1C]">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>{icon}</svg>
                    </div>
                    <span className="text-[13px] font-semibold uppercase tracking-wide text-[#2A1F14]">{label}</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m9 18 6-6-6-6" /></svg>
                </Link>
              ))}
              ...
              <div>
                <button
                  className="flex w-full items-center justify-between rounded-xl border border-[#EAE3DA] bg-white px-3 py-2.5 shadow-sm"
                  onClick={() => setMoldsOpen((o) => !o)}
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[#3B2A1C]">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    ...
                    <span className="text-[13px] font-semibold uppercase tracking-wide text-[#2A1F14]">Molds</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-3.5 w-3.5 text-neutral-400 transition-transform ${moldsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="m6 9 6 6 6-6" />
                  </svg>
                </button>
                {moldsOpen && (
                  <div className="mt-1 ml-11 flex flex-col gap-1 rounded-xl border border-[#EAE3DA] bg-white/80 p-2">
                    {categories.map((cat) => (
                      <Link key={cat.id} href={`/molds/${cat.slug}`} onClick={close} className="flex items-center gap-2 rounded-lg px-2 py-2">
                        <div className="h-7 w-7 flex-shrink-0 overflow-hidden rounded-full bg-neutral-200">
                          {cat.banner_image ? (
                            <SmartImage src={cat.banner_image} alt={cat.name} width={28} height={28} className="h-full w-full object-cover" />
                          ) : (
                            <div className="flex h-full w-full items-center justify-center text-[10px] text-neutral-400">🧁</div>
                          )}
                        </div>
                        <span className="text-xs text-[#2A1F14]">{cat.name}</span>
                      </Link>
                    ))}
                    <Link href="/molds" onClick={close} className="rounded-lg px-2 py-2 text-xs font-semibold text-[#3B2A1C]">View All Molds →</Link>
                  </div>
                )}
              </div>
            </nav>
